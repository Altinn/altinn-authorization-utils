...
internal static class XmlCommentsNodeNameHelper
{
    ...
    {
        Debug.Assert(fieldOrPropertyInfo.DeclaringType is not null);
    ...
    private static string QualifiedNameFor(Type type, bool expandGenericArgs = false)
    {
        if (type.IsArray)
        {
            ...
            return elementType.IsGenericParameter ? $"`{elementType.GenericParameterPosition}[]" : $"{QualifiedNameFor(elementType, expandGenericArgs)}[]";
        }
