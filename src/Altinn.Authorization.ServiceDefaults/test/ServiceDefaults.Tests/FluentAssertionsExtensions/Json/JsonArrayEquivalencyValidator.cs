...
namespace Altinn.Authorization.ServiceDefaults.Tests.FluentAssertionsExtensions.Json;
...
internal ref struct JsonArrayEquivalencyValidator
{
    ...
    private void AssertElementGraphEquivalencyWithStrictOrdering()
    {
        ...
        foreach (int index in Enumerable.Range(0, _expectations.Length))
        {
            ...
            using var _ = _context.Tracer.WriteBlock(member => string.Create(CultureInfo.InvariantCulture,
            ...
            if (!succeeded)
            {
                ...
                {
                    _context.Tracer.WriteLine(member => string.Create(CultureInfo.InvariantCulture,
    ...
    private bool LooselyMatchAgainst(JsonElement expectation, int expectationIndex)
    {
        ...
        GetTraceMessage getMessage = member => string.Create(CultureInfo.InvariantCulture,
