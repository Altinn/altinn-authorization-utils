    ...
    : IXmlDocProvider
{
    ...
    public bool TryGetXmlDoc(ParameterInfo parameter, [NotNullWhen(true)] out XPathNavigator? node)
    {
        ...
        if (method is null || !TryGetXmlDoc(method, out var methodNode))
        {
            node = null;
            return false;
        }

        var parameterNodes = methodNode.Select("param[@name]");
        foreach (XPathNavigator paramNode in parameterNodes)
        {
            if (paramNode.GetAttribute("name", string.Empty) == parameter.Name)
            {
                node = paramNode;
                return true;
            }
